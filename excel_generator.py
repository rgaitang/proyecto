...
def _normalizar(nombre):
    """Normaliza un nombre para comparaciones (minúsculas, sin acentos, partes sueltas)."""
    nombre = nombre.lower()
    nombre = nombre.replace('ñ', 'n').replace('á', 'a').replace('é', 'e') \
                   .replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
    # Quitar duplicados de palabras y nombres incompletos tipo "Margarita"
    partes = nombre.split()
    resultado = []
    for p in partes:
        if p and not resultado:
            resultado.append(p)
        elif p and p != resultado[-1]:
            resultado.append(p)
    return ' '.join(resultado)


def _normalizar_palabras(nombre):
    """Conjunto de palabras del nombre (para vincular por apellido + nombre)."""
    n = _normalizar(nombre)
    return set(n.split())


def _vincular_cedula(nombre_sistema, empleados_plantilla):
    """Busca la cédula del empleado en la lista de la plantilla por coincidencia
    de nombre. Devuelve (cedula, nombre_completo_plantilla)."""
    palabras = _normalizar_palabras(nombre_sistema)
    # Excluir nombres demasiado cortos / genéricos
    apellidos = [p for p in palabras]
    mejor = ('', '')
    mejor_puntos = 0
    for ep in empleados_plantilla:
        ep_words = _normalizar_palabras(ep['nombre'])
        if not ep_words:
            continue
        comunes = palabras & ep_words
        if comunes:
            puntos = len(comunes)
            if puntos > mejor_puntos:
                mejor_puntos = puntos
                mejor = (ep['cedula'], ep['nombre'])
    # Solo aceptar si hay al menos 2 palabras en comun o 1 palabra de 5+ letras
    if mejor_puntos >= 2 or (mejor_puntos == 1 and any(len(w) >= 5 for w in comunes)):
        return mejor
    return ('', '')


def _buscar_empleado_sistema(nombre_plantilla, index_sistema):
    """Busca en el indice de empleados del sistema ({nombre_normalizado: [(id, regs)]})
    el que mejor coincida con el nombre real de la plantilla. Exige al menos 2
    palabras en comun y se queda con el mayor numero de coincidencias (desempate
    por nombre mas largo). Devuelve el dict de registros o {} si no hay match."""
    palabras = _normalizar_palabras(nombre_plantilla)
    mejor_puntos = 0
    mejor_regs = None
    mejor_len = -1
    for key, candidatos in index_sistema.items():
        comunes = palabras & set(key.split())
        if not comunes:
            continue
        puntos = len(comunes)
        largo = len(key.split())
        # mayor coincidencia; ante empate, preferir el nombre del sistema mas largo
        if puntos > mejor_puntos or (puntos == mejor_puntos and largo > mejor_len):
            mejor_puntos = puntos
            mejor_len = largo
            mejor_regs = candidatos[0][1]
    if mejor_puntos >= 2:
        return mejor_regs or {}
    return {}
